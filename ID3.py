...
def informationGain(data, features, feature_for_gain, target_feature):
    ...
    for each_value in frequency_of_values.keys():
        probability_of_value = frequency_of_values[each_value] / sum(frequency_of_values.values())
...
def entropy(data, features, feature_for_entropy):
    ...
    for each_frequency in frequency_of_values.values():
        #print each_frequency, len(data)
        frequency_by_length = float(each_frequency)/len(data)
        #print frequency_by_length
...
def gainRatio(data, features, feature_for_gain, target_feature):
    gain_ratio = informationGain(data, features, feature_for_gain, target_feature) / entropy(data, features, feature_for_gain)
    return gain_ratio


#Returns the best feature depending on the maximum information gain
def bestFeature(data, features, target_feature):
    if len(features) == 1:
        return ;
...
def buildTreeID3(data, features, target_feature):
    root = Node
    ...
    #If the only feature left is the target Variable
        ...
        return countMaxTarget(target_list)

    #Else build the decision tree by choosing the best feature
    else:
        best_feature = bestFeature(data, features, target_feature)
        ...
        for value in values_of_best_feature:
            ...
            sub_tree = buildTreeID3(examples, new_feature_list, target_feature)
